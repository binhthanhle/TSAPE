...
def process(input_image, params, model_params, old_peaks, scale_search,canvas2):
    ...
    for part in range(18):
        ...
        peaks_binary = np.logical_and.reduce(
            (map >= map_left, map >= map_right, map >= map_up, map >= map_down, map > 0.35)) # ori: params['thre1'] -->0.35
        ...
        if (nA != 0 and nB != 0):
            ...
            for i in range(nA):
                for j in range(nB):
                    ...
                    criterion1 = len(np.nonzero(score_midpts > params['thre2'])[0]) > 0.5 * len(score_midpts)
...
<<<<<<< HEAD
    # if canvas2 is None:
    #     flag = 1
    canvas2 = np.zeros(input_image.shape, np.uint8)
    # else:
    flag = 2
=======
    if canvas2 is None:
        ...
        flag = 2
>>>>>>> b42272f0fb72e830dcd116a40c0510566db668c5
...
>>>>>>> b42272f0fb72e830dcd116a40c0510566db668c5
    for i in range(18):
        for j in range(len(all_peaks[i])):
            dictoutput[text[i]] =  all_peaks[i][j][0:2]
            cv2.circle(canvas, all_peaks[i][j][0:2], 4, colors[i], thickness=-1)
            cv2.putText(canvas, text[i], all_peaks[i][j][0:2], cv2.FONT_HERSHEY_SIMPLEX, 0.5, colors[i], 2, cv2.LINE_AA)
            cv2.circle(canvas2, all_peaks[i][j][0:2], 4, colors[i], thickness=-1)
<<<<<<< HEAD
            cv2.putText(canvas2, text[i], all_peaks[i][j][0:2], cv2.FONT_HERSHEY_SIMPLEX, 0.5, colors[i], 2,
                        cv2.LINE_AA)
            # if flag == 1:
            #     cv2.putText(canvas2, text[i], all_peaks[i][j][0:2], cv2.FONT_HERSHEY_SIMPLEX, 0.5, colors[i], 2, cv2.LINE_AA)
=======
            if flag == 1:
                cv2.putText(canvas2, text[i], all_peaks[i][j][0:2], cv2.FONT_HERSHEY_SIMPLEX, 0.5, colors[i], 2, cv2.LINE_AA)
>>>>>>> b42272f0fb72e830dcd116a40c0510566db668c5
    ...
    for i in range(17):
        for n in range(len(subset)):
            ...
            if flag == 1:
                cv2.fillConvexPoly(canvas2, polygon, [238,233,233])
                canvas2 = cv2.addWeighted(canvas2, 0.4, canvas2, 0.6, 0)
...
<<<<<<< HEAD
    ...
    return canvas, all_peaks,canvas2,dictoutput
=======

    return canvas, all_peaks,canvas2
>>>>>>> b42272f0fb72e830dcd116a40c0510566db668c5
...
if __name__ == '__main__':
    ...
    parser.add_argument('--folder', type=str, required=True, help='input video file name')
    ...
    for r, d, f in os.walk(folder):
        for file in f:
            ...
            video_output2 = output_path + video + str(start_datetime)+ "_skeleton" + output_format

            # brgname = video.split("_")[0]
            # bgfolder = '../data/Background/'
            #
            # bgrndImage = cv2.imread(bgfolder+brgname+'.png', cv2.IMREAD_GRAYSCALE)
            ...
            # bgwidth = bgrndImage.shape[1]
            # bgheight = bgrndImage.shape[0]
            #
            # bgcropped = bgrndImage[:,int(bgwidth * factor): int(bgwidth * (1 - factor))]
            # bgcropped = bgcropped[int(bgheight * factor2):int(bgheight * (1 - factor3)),:]
            # bgcropped = cv2.resize(bgcropped, (0, 0), fx=1.0 / resize_fac, fy=1.0 / resize_fac,
            #                      interpolation=cv2.INTER_CUBIC)
            #
            # bgcropped = cv2.GaussianBlur(bgcropped, (5, 5), 0)
            ...
            out2 = cv2.VideoWriter(video_output2, fourcc, output_fps, (cropped.shape[1], cropped.shape[0]))
            i = 0  # default is 0
            # resize_fac = 1
...
<<<<<<< HEAD
            columnnames = ['Nose', 'Neck',
                    'RShoulder', 'RElbow', 'RWrist', 'LShoulder', 'LElbow', 'LWrist',
                    'RHip', 'RKnee', 'RAnkle', 'LHip', 'LKnee', 'LAnkle',
                    'REye', 'LEye', 'REar', 'LEar']

            df_timeseries_x = pd.DataFrame([], columns=columnnames)
            df_timeseries_y = pd.DataFrame([], columns=columnnames)
...
<<<<<<< HEAD
                    canvas, all_peaks , canvas2,dictoutput= process(cropped, params, model_params, old_peaks, scale_search,canvas2)


=======
                    canvas, all_peaks , canvas2= process(cropped, params, model_params, old_peaks, scale_search,canvas2)
>>>>>>> b42272f0fb72e830dcd116a40c0510566db668c5

                    old_peaks = all_peaks

                    numpy_horizontal = np.hstack((canvas, canvas2))
...
<<<<<<< HEAD
                    ...
                    for eachkey in list(dictoutput.keys()):
                        print("key:", eachkey, "positions:", dictoutput.get(eachkey))
                        dictx[eachkey] = dictoutput.get(eachkey)[0]
                        dicty[eachkey] = dictoutput.get(eachkey)[1]
                    ...
                    df_timeseries_x = df_timeseries_x.append(dictx, ignore_index=True)
                    df_timeseries_y = df_timeseries_y.append(dicty, ignore_index=True)
=======
                    df_timeseries_x = df_timeseries_x.append(pd.DataFrame([e[0] for each in all_peaks for e in each]).T,
                                                             ignore_index=True)
                    df_timeseries_y = df_timeseries_y.append(pd.DataFrame([e[1] for each in all_peaks for e in each]).T,
...
<<<<<<< HEAD
            ...
            #     columns={0: 'Nose', 1: 'Neck',
            #              2: 'RShoulder', 3: 'RElbow', 4: 'RWrist', 5: 'LShoulder', 6: 'LElbow', 7: 'LWrist',
